...
const MODULES = {
  ...
  autonomous: { title: 'AUTONOMOUS MODE', badge: 'NEXT STEP ORCHESTRATION · BETA', hint: 'agent workflow', tools: [] },
...
function renderAutonomousTab() {
  document.getElementById('tool-grid').innerHTML = '';
  document.getElementById('opts-row').innerHTML =
    '<span class="opts-hint">// autonomous mode entry is live — backend planner comes next</span>';
  document.getElementById('cmd-text').textContent = 'Autonomous mode will inspect outputs, choose the next tool, and chain scans automatically.';
  document.getElementById('output-area').innerHTML = `
    <div class="agent-tab autonomous-tab">
      <div class="agent-card autonomous-card">
        <div class="agent-card-title">AUTONOMOUS MODE</div>
        <p>This tab is the foundation for the upcoming self-driving workflow. It will analyze current scan artifacts and decide the next logical action without manual tool switching.</p>
        <div class="agent-btn-row">
          <button class="btn btn-auto" disabled>STEP 1 COMPLETE</button>
        </div>
      </div>
      <div class="agent-card">
        <div class="agent-card-title">PLANNED PIPELINE</div>
        <p>1. Read existing files in <code>output/</code>.<br>2. Call <code>/api/agent/next_step</code>.<br>3. Launch the recommended scan.<br>4. Repeat until no next step remains.</p>
      </div>
      <div class="agent-card">
        <div class="agent-card-title">NEXT IMPLEMENTATION STEP</div>
        <p>The next backend task is the decision endpoint. After that, the frontend runner can recursively execute each recommended step.</p>
      </div>
    </div>`;
}
