...
def build_command(module, tool, target, options=None):
    ...
    cmds = {
      ...
      "params": {
        ...
        "ffuf-params":
          guard(f"[ -f {WL_PAR} ] || {{ echo '[ERR] Wordlist missing — sudo apt install seclists'; exit 1; }} && while read url; do ffuf -u \"${{url}}?FUZZ=PentAItest\" -w {WL_PAR} -mc 200 -ac -silent 2>/dev/null | tee -a output/params_found.txt; done < output/Alive200.txt",
                tools="ffuf", needs="output/Alive200.txt"),
      },
